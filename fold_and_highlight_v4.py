...
from welcome import welcome_fold,welcome
from os import path,system

welcome()
welcome_fold()

if not (path.isfile("motif_start") and path.isfile("motif_end")):
    print("\n**ERROR: Files missing. Check if 'motif_start' and 'motif_end' files are present in this directory.**\n")
    exit()

choice = int(input("\nEnter the total number of files to be processed. "))  ##User input for the number of files to be processed.
...
#system("echo why; shopt -s extglob") ##This does not work in a python instance.
...
for x in range(choicex,choice+1):
	print(f"******************************* ITERATION # {x} *********************************")
	n = system(f'mfold SEQ"=prec{x}.fasta"') ##mfold command to create folds with default options.
	system("bash remover.sh") ##Script executed to remove redundancy and additional files generated by mfold to save storage space.
	#system('rm -v !(*.py|*.fasta|*coord|*.ct|*.EPS|*.jar') ##Does not work in this python instance. These are the contents of the "remover.sh" shell script.
	if n == 0:
		try:
			flag = 0
			...
			for seq in open(f"prec{x}.ct"):
				...
				if flag == 1:
					file.write(seq)
			file.close()
			system(f'java -cp VARNAv3-93.jar fr.orsay.lri.varna.applications.VARNAcmd -i new_prec{x}.ct -o prec{x}.EPS -highlightRegion "{int(start[x-1])+1}-{end[x-1]}:fill=#FF0000"') ##Command to highlight the ".ct" extension files according to the miRNA motif start/end coordinates.
		except:
			err_log.write(f"Corrupt CT : prec{x}.fasta")
			system(f"bash err_ct.sh prec{x}.ct new_prec{x}.ct new_prec{x}")
			system(f'java -cp VARNAv3-93.jar fr.orsay.lri.varna.applications.VARNAcmd -i new_prec{x}.ct -o prec{x}.EPS -highlightRegion "{int(start[x-1])+1}-{end[x-1]}:fill=#FF0000"')
	else:
